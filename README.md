...
### 1. 附加上下文恢复 (With-Context Continuation)
在执行回合中，拦截由于验证失败或工具调用错误引发的 `stop`。通过重试消息进行错误修正，附带最高 **5 次的指数退避**，以防范网络抖动和服务器抽风。

### 2. 无上下文自动恢复 (Without-Context Recovery)
拦截当代理出现严重的上下文满载（`context_overflow`）或被强制阻塞（`blocked`）后的中断。开启一个隐式会话，让 LLM 主动对失败现场进行独立诊断。诊断完毕后，自动向 GSD 内核发射 `/gsd auto` 以满血复活 `auto-mode`。

### 3. 工具死循环防护 (Tool Call Loop Guards)
精准监控连续发起**相同输入**的无效工具调用。如果连续 4 次徒劳无功，本插件会向 LLM 投喂一段虚假的 “成功” 输出，用大写文字强制要求它 “重新思考方案”，从根源切断系统级崩溃。

### 4. 语义校验拦截补丁 (Semantic Validation Patch)
...
在下次运行 `/gsd auto` 时，本插件将自动静默生效，享受自愈保护。
...
所有一切都被局限在这个单文件、轻量级的事件处理器中。没有任何繁重的依赖编译，只有绝对确定的防御。
